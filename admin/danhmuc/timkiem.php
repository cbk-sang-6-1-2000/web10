...
<html lang="en">
   ...
   <body class="main-layout">
	...
	<!-- new collection section start -->
  	<div class="collection_text" style =";font-size: 30px;font-family: 'Times New Roman', Times, serif; padding : 15px 0px; margin-top: 106px">Quản Lý Danh Mục</div>
...
if (isset($_REQUEST['ok'])) 
{
    ...
    {
        $sql = "select category.idCategory, category.CategoryName, COUNT(equipment.idCategory) from category left join equipment on category.idCategory = equipment.idCategory where category.CategoryName LIKE '%$search%' GROUP BY category.idCategory";
        $danhmuc_list = executeResult($sql);
        if ($danhmuc_list > 0 && $search != "") 
        {
	        ...
	        echo '<tr>
                    ...
                    <td>'.$item['COUNT(equipment.idCategory)'].'</td>
                    <td>
                        <a href="them.php?idCategory='.$item['idCategory'].'"><button class="btn btn-warning">Sửa</button></a>
                    </td>
                    <td>
                        <button class="btn btn-danger" onclick="danhmuc_list('.$item['idCategory'].')">Xoá</button>
                    </td>
			    </tr>';
            }
        }
...
?>
    ...
    <script type="text/javascript">
		function danhmuc_list(idCategory) {
